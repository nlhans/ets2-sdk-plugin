...
ets2TelemetryMap_t* telem_ptr;
/**
 * \brief Name/Location of the Shared Memory
 */
const wchar_t* ets2_mmf_name = ETS2_PLUGIN_MMF_NAME;
...
SCSAPI_RESULT scs_telemetry_init(const scs_u32_t version, const scs_telemetry_init_params_t*const params) {
    ...
    telem_mem = new SharedMemory(ets2_mmf_name, ETS2_PLUGIN_MMF_SIZE);
    ...
    telem_ptr = static_cast<ets2TelemetryMap_t*>(telem_mem->GetBuffer());

    if (telem_ptr == nullptr) {
        return SCS_RESULT_generic_error;
    }

    memset(telem_ptr, 0, ETS2_PLUGIN_MMF_SIZE);
